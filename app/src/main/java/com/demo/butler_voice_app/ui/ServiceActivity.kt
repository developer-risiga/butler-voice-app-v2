...
class ServiceActivity : ComponentActivity() {
        ...
        sarvamSTT.startListening(
            onResult = { transcript ->
                runOnUiThread {
                    ...
                    else speak("Sorry, I didn't catch that. Which service do you need?") { startListeningForSector() }
                }
            },
            onError = { runOnUiThread { speak("Something went wrong. Please try again.") { startListeningForSector() } } }
        )
    }
        ...
        sarvamSTT.startListening(
            onResult = { transcript ->
                runOnUiThread {
                    ...
                    speak("Please say 1, 2 or 3 to select a provider, or say filter by rating, nearest, or cheapest.") { startListeningForSelection() }
                }
            },
            ...
            onResult = { transcript ->
                runOnUiThread {
                    ...
                    when {
                        ...
                        else -> speak("Say yes to confirm the booking or no to go back.") { startListeningForBookingConfirm(provider) }
                    }
                }
            },
    ...
    private fun confirmBooking(provider: ServiceProvider) {
        ...
        lifecycleScope.launch {
            try {
                ...
                Log.e("ServiceActivity", "Failed to save booking: ${e.message}")
            }
        }
    }
    ...
    private fun applyFilter(filter: ServiceFilter) {
        ...
        lifecycleScope.launch {
            ...
            speak("Showing $sortName ${sector.displayName} providers. Say 1, 2, or 3 to choose.") { startListeningForSelection() }
        }
    }
    ...
    private fun goBack() {
        when (screenState) {
            ...
            is ServiceScreenState.ProviderList, is ServiceScreenState.Prescription -> {
                screenState = ServiceScreenState.SectorHome; speakServicePrompt()
            }
            else -> finish()
        }
    }
    ...
    // Uses SupabaseClient constants — no BuildConfig Supabase fields needed
    ...
    private suspend fun saveBookingToSupabase(
        ...
        Log.d("ServiceActivity", "Booking saved: ${response.code} $bookingId")
    }
}
