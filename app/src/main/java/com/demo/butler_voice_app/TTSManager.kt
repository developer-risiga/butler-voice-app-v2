...
class TTSManager(
     ...
     * Replace Hindi/Telugu/Tamil values with real voice IDs from your ElevenLabs account.
     * Get Hindi voices at: https://elevenlabs.io/voice-library (search "Hindi")
     */
    private val voiceMap = mapOf(
        "en" to voiceId,
        "hi" to "pqHfZKP75CvOlQylNhV4",   // Meera (Hindi) — change to your preferred
        "te" to voiceId,                    // replace when you have a Telugu voice
        "ta" to voiceId,                    // replace when you have a Tamil voice
        "pa" to voiceId                     // replace when you have a Punjabi voice
    )
    ...
    private fun callWithRetry(
        ...
        client.newCall(request).enqueue(object : Callback {
            override fun onFailure(call: Call, e: IOException) {
                ...
                if (attempt < 2) callWithRetry(request, text, language, onDone, attempt + 1)
                else { Log.w("TTS", "Falling back to system TTS"); speakFallback(text, language, onDone) }
            }
            override fun onResponse(call: Call, response: Response) {
                if (!response.isSuccessful) {
                    Log.e("TTS", "ElevenLabs error ${response.code}")
                    if (attempt < 2) callWithRetry(request, text, language, onDone, attempt + 1)
                    else speakFallback(text, language, onDone)
                    return
                }
        ...
        try {
            ...
            mediaPlayer = MediaPlayer().apply {
                ...
                setOnCompletionListener {
                    it.release(); mediaPlayer = null; file.delete(); onDone?.invoke()
                }
                start()
            }
        } catch (e: Exception) {
    ...
    private fun speakFallback(text: String, language: String, onDone: (() -> Unit)?) {
        ...
        fallbackTts?.setOnUtteranceProgressListener(object : UtteranceProgressListener() {
            ...
            override fun onDone(u: String?)   { if (u == id) onDone?.invoke() }
            override fun onError(u: String?)  { if (u == id) onDone?.invoke() }
        })
    ...
    fun shutdown() {
        ...
        fallbackTts?.shutdown(); fallbackTts = null
    }
}